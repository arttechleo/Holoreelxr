...
export class ReactionHud {
  ...
  private readonly PANEL_W = 0.28;
  private readonly PANEL_H = 0.20;
  private readonly OFFSET = new THREE.Vector3(0, 0.16, 0);
  ...
  getCounts() { return { like: this.likeCount, heart: this.heartCount }; }

  bump(kind: 'like'|'heart') {
    if (kind === 'like') this.likeCount++; else this.heartCount++;
    this.redraw(); this.spawnChip(kind); this.show();
  }
  ...
  private redraw() {
    ...
    ctx.globalAlpha = 0.22; ctx.fillRect(18, 102, c.width - 36, 2); ctx.globalAlpha = 1;
    ...
    this.wrap(ctx, 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam eget hendrerit metus.', 22, 160, c.width - 44, 22);
    this.wrap(ctx, 'Integer faucibus magna non tincidunt mattis, purus lorem gravida augue, nec viverra nibh enim eget velit.', 22, 206, c.width - 44, 22);
    ctx.globalAlpha = 1;
  ...
  private spawnChip(kind: 'like'|'heart') {
    const canv = document.createElement('canvas'); canv.width = 256; canv.height = 128;
    ...
    cx.fillStyle='#fff'; cx.font='700 42px system-ui, -apple-system, Segoe UI, Roboto, sans-serif';
    ...
    const chipW = this.PANEL_W * 0.55; const aspect = canv.height / canv.width;
    sprite.scale.set(chipW, chipW * aspect, 1); sprite.position.set(0, this.PANEL_H * 0.35, 0.002);
